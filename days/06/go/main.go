...
func part1(input string) int {
	groupStrings := strings.Split(input, "\n\n")

	groups := make([]map[int32]bool, 0, len(groupStrings))
	for _, groupString := range groupStrings {
		group := make(map[int32]bool)
		for _, personString := range strings.Split(groupString, "\n") {
		...
		for _, personString := range strings.Split(groupString, "\n") {
			for _, answer := range personString {
				if first {
					...
					group = intersect(group, personString)
				}
			}
